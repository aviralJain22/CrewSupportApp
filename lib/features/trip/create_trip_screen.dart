...
enum _ExitChoice { saveAndExit, exitWithoutSaving, cancel }

class CreateTripScreen extends GetWidget<CreateTripController> {
    ...
    final choice = await showCupertinoDialog<_ExitChoice>(
      ...
      builder: (_) => Theme(
        ...
        child: CupertinoAlertDialog(
          ...
          content: Text('This trip has not been saved as a draft yet. Do you want to save before exiting?'),
          actions: [
  ...
  Future<void> _pickEndDate(BuildContext context) async {
    ...
    // But when editing End Date, the picker should initially highlight the
    // previously selected End Date instead of jumping back to Start Date.
    ...
    if (endDateText.isNotEmpty) {
      try {
        ...
        // Keep Start Date as fallback if End Date text is malformed.
      }
    }
              ...
              child: Row(
                ...
                children: [
                  ...
                  CupertinoButton(
                    ...
                    onPressed: () {
                      // CupertinoPicker does not call onSelectedItemChanged for the initially highlighted item.
                      // Commit the currently highlighted/default value when Done is tapped so "<10 miles"
                      // works even if the user does not scroll first.
                      controller.setRadiusDisplayAndValue(options[selectedIndex]);
  ...
  void _navigateToRoleSearchScreens(BuildContext context) {
    ...
    if (controller.selectList.length == 1) {
      if (controller.selectList.contains('Captain')) {
        // Get.toNamed(
        //   AppRoutes.searchByCaptain,
        //   arguments: {
        //     'membershipId': controller.derivedSingleRoleMembershipId,
        //     'oldCrewMemberId': controller.oldCrewMembersKey,
        ...
        //     'totalTimeType': first.totalTimeAircraftId,
        //     'picTimeType': first.picTimeAircraftId,
        //     'picTime': first.aircraftPiCtime,
        //     'medicalClass': first.faaMedical.toString(),
        //     'validPassport': first.havePassport,
        //     'ratingType': first.rating,
        //     'continentExp': first.continent,
        //     'monthTraining': first.isReqPrev12MonthTraining.toString(),
        //     'rate': first.pilotRate.round(),
        //     'boolSIC': first.secondInCommand,
        //     'airCraftID': first.fkAircraftId,
        //     'boolFI': first.flightInstructor,
        //     'boolFA': first.flightAttendant,
        //     'boolCAP': first.captain,
        //     'oceanicExp': first.ocean,
        //     'tripId': first.pkTripId,
        //     'miles': first.radius,
        //     'rating': int.tryParse(first.ratingCount) ?? 0,
        //     'ratingAirCraftType': first.ratingAirCraftType,
        ...
        //   MaterialPageRoute(
        //     builder: (_) => SearchBySICScreen(
        //       membershipId: controller.derivedSingleRoleMembershipId,
        //       oldCrewMemberId: controller._oldCrewMembersKey,
        //       startDate: controller.startDateController.text,
        //       endDate: controller.endDateController.text,
        //       selectList: controller.selectList,
        //       totalTime: first.AircraftTotalTimeSIC,
        //       totalTimeType: first.TotalTimeAircraftIdSIC,
        //       medicalClass: first.FAAMedicalSIC?.toString(),
        //       validPassport: first.HavePassportSIC,
        //       ratingType: first.ratingSIC,
        //       continentExp: first.ContinentSIC,
        //       monthTraining: first.IsReqPrev12MonthTrainingSIC?.toString(),
        //       boolSIC: first.secondInCommand,
        //       boolFI: first.flightInstructor,
        //       boolFA: first.flightAttendant,
        //       tripId: first.pkTripId,
        //       miles: first.radius,
        //       ratingAirCraftType: first.aircraftType,
        //       ratingCount: first.ratingCount,
        //       fAAMedical: first.FAAMedicalSIC?.toString(),
        //       airCraftId: first.fkAircraftId,
        //       rate: first.sicRate?.round().toString(),
        //       isTripOutOfDate: true,
        //     ),
        //   ),
        // );
        ...
        //       boolFI: first.flightInstructor,
        //       tripId: first.pkTripId,
        //       ratingCount: int.tryParse(first.ratingCount ?? '0') ?? 0,
        ...
        // Navigator.of(context).push(
        //   MaterialPageRoute(
        //     builder: (_) => SearchByFIScreen(
        //       startDate: controller.startDateController.text,
        //       endDate: controller.endDateController.text,
        //       selectList: controller.selectList,
        //       tripId: first.pkTripId,
        //       ratingCount: int.tryParse(first.ratingCount ?? '0') ?? 0,
        //       rate: first.fiRate?.round().toString(),
        //       radiusValue: first.radius,
        //       airCraftID: first.fkAircraftId,
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      appBar: AppBar(
        ...
        title: Obx(
          () => Text(
            controller.isTripOutOfDate.value ? "Update Trip" : "Create Trip",
                      ...
                      Expanded(
                        child: ListView(
                          children: [
                            ...
                            _rowTwoCol(
                              left: "Start Date",
                              rightChild: GetBuilder<CreateTripController>(
                                id: 'dates',
  ...
  Widget _createTripButton(BuildContext context) {
    return MaterialButton(
      ...
      onPressed: () async {
        ...
        if (controller.shouldShowEmptyRadiusWarning()) {
          await showCupertinoDialog<void>(
            context: context,
            builder: (_) => Theme(
              data: ThemeData.dark(),
              child: CupertinoAlertDialog(
                title: const Text('Radius not selected'),
                content: const Text(
                  'No radius selected. Tap next button again to search all or specify desired crew proximity.',
                ),
                actions: [
                  CupertinoDialogAction(
                    isDefaultAction: true,
                    onPressed: () {
                      controller.setRadiusDisplayAndValue('Any distance');
                      Navigator.of(context).pop();
                    },
                    child: Text(
                      'OK',
        ...
        try {
          ...
          // Merge CreateTripScreen fields into shared draft WITHOUT wiping changes from later screens
          ...
          if (controller.createdCaptain == true) {
            Get.toNamed(
              AppRoutes.requiredExperienceCaptain,
              arguments: {
                'boolCAP': controller.createdCaptain,
                'miles': milesArg,
                'selectList': roles,
                'tripId': tripIdArg,
                'boolSIC': controller.createdSecondInCommand,
                'boolFA': controller.createdFlightAttendant,
                'boolFI': controller.createdFlightInstructor,
                'airCraftID': airId,
                'ratingAirCraftType': acText,
                'startDate': controller.startDateController.text,
                'endDate': controller.endDateController.text,
              },
            );
          } else if (controller.createdSecondInCommand == true) {
            Get.toNamed(
          ...
          } else if (controller.createdFlightAttendant == true) {
            Get.toNamed(
          ...
          } else if (controller.createdFlightInstructor == true) {
            // Get.toNamed(
            ...
            await showMyDialog(context, "No matching role found!");
          }
        } catch (e) {
      ...
      child: Text("NEXT", style: TextStyle(fontSize: 10.spV2, fontWeight: FontWeight.w600)),
    );
  }
}